...
def main():
    ...
    for i in range(start_iter, args.num_steps):
        ...
        # print(src_img.shape)
        # print(trg_img.shape)
        src_in_trg = FDA_source_to_target_np( src_img, trg_img, L=args.LB )            # src_lbl
        trg_in_trg = trg_img

        # 2. subtract mean
        # print('src_in_trg:', type(src_in_trg))
        # print('trg_in_trg: ',type(trg_in_trg))
        # print('mean_img: ',type(mean_img))
        src_img = torch.from_numpy(src_in_trg) - mean_img                                 # src, src_lbl
        ...
        src_seg_score = model(src_img.type(torch.cuda.FloatTensor), lbl=src_lbl, weight=class_weights, ita=args.ita)      # forward pass
        ...
        trg_seg_score = model(trg_img.type(torch.cuda.FloatTensor), lbl=trg_lbl, weight=class_weights, ita=args.ita)      # forward pass
